...
import { z } from 'zod';
import { problemStatementQuestions, generateProblemStatement, Question } from '@/app/lib/questions/problem-statement';
...
interface ProblemStatementModuleProps {
  ventureId: string;
  onComplete: (artifact: any) => void;
  existingArtifact?: any;
}

export default function ProblemStatementModule({ 
  ventureId, 
  onComplete,
  existingArtifact 
}: ProblemStatementModuleProps) {
  ...
  const allQuestions = [...problemStatementQuestions, ...additionalQuestions];
  ...
  const handleComplete = async (finalAnswers: Record<string, string>) => {
    const problemStatement = generateProblemStatement(finalAnswers);
      ...
      <div className="max-w-3xl mx-auto p-6">
        <div className="bg-green-50 border border-green-200 rounded-lg p-6 text-center">
          ...
          <p className="text-green-700">
            Your problem statement has been saved. You can now move on to creating customer personas.
          </p>
        </div>
        <div className="mt-6 p-6 bg-gray-50 rounded-lg">
          <h3 className="font-semibold mb-3">Your Problem Statement:</h3>
          <div className="prose prose-sm max-w-none">
            <pre className="whitespace-pre-wrap text-sm">
              {generateProblemStatement(answers)}
            </pre>
          </div>
        </div>
      </div>
